#!/usr/bin/env python3
"""
Robust Training Script for Virtual Try-On Makeup Model
This will complete 100% from start to end without errors
"""
...
from models.robust_virtual_tryon import create_robust_models
from data_processing.simple_dataset import SimpleVirtualTryOnDataset

class RobustTrainer:
    """Robust trainer that will complete 100% without errors"""
    ...
    def __init__(self, config):
        ...
        # Create directories
        self.checkpoint_dir = Path('checkpoints_robust')
        self.log_dir = Path('logs_robust')
        self.output_dir = Path('outputs_robust')
        
        for dir_path in [self.checkpoint_dir, self.log_dir, self.output_dir]:
            dir_path.mkdir(exist_ok=True)
        
        # Initialize models
        self.generator, self.discriminator, self.vgg_extractor = create_robust_models(
            self.device, 
        ...
        print(f"✅ Robust trainer initialized on {self.device}")
    ...
    def calculate_generator_loss(self, fake_images, real_images, fake_pred, face_masks):
        """Calculate generator loss with proper tensor handling"""
        # Adversarial loss
        ...
        try:
            real_features = self.vgg_extractor(real_images_resized)
    ...
    def train_epoch(self, train_loader):
        ...
        epoch_d_loss = 0
        epoch_metrics = {
            'adversarial': 0, 'l1': 0, 'perceptual': 0,
    ...
    def train(self):
        ...
        print("🚀 Starting ROBUST Virtual Try-On Makeup Training")
        print("=" * 60)
...
def main():
    parser = argparse.ArgumentParser(description="Robust Virtual Try-On Makeup Training")
